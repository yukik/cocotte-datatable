...
function setEventListener(dt) {
  function listener (eventName, row, fieldName) {
    switch(eventName) {
    case 'added':
    ...
    case 'updated':
      ...
      if (dt.liveList && dt.test(row) !== exists) {
        ...
        dt.emit('all', 'updated', row, fieldName);
        var reList = (function(){
          if (!dt.liveList) {
            return false;
          }
    ...
    case 'threw': // 入力エラー
      if (dt.exists(row)) {
        dt.emit('threw', row, fieldName);
        dt.emit('all', 'threw', row, fieldName);
      }
      break;
    case 'removed':
      ...
      break;
    default:
      throw new Error(eventName + '捕捉漏れ');
    }
  }
  dt.ds.on('all', listener);
  return listener;
}
...
Datatable.prototype.destroy = function destroy () {
  this.ds.removeListener('all', this._dsListener);
  this.ds = null;
  this.rows = null;
  this._liveList = null;
...
Object.defineProperties(Datatable.prototype, {
  ...
  liveList: {
    ...
    set: function (live) {
      ...
      if (live !== this._liveList) {
        this._liveList = live;
        this.emit('set liveList', live);
        this.emit('all', 'set liveList', live);
        if (live) {
          this.list();
        }
      }
  ...
  currentRow: {
    ...
    set: function (value) {
      ...
      var from = this._currentRow;
      if (from !== value) {
        this._currentRow = value;
        this.emit('moved', from, value);
        this.emit('all', 'moved', from, value);
      }
    },
    enumerable: true,
    configurable: false
...
Datatable.prototype.setCondition = function setCondition (condition) {
  ...
  if (!compare(condition, this._condition)) {
    ...
    this.emit('all', 'set condition', condition);
    if (this.liveList) {
      this.list();
    }
  }
...
Datatable.prototype.setOrder = function setOrder(order) {
  ...
  if (!compare(order, this._order)) {
    this._order = order;
    this.emit('set order', order);
    this.emit('all', 'set order', order);
    if (this.liveList) {
      this.list();
    }
  }
...
Datatable.prototype.setRange = function setRange(from, to) {
  ...
  this._from = from;
  this._to = to;
  this.emit('set range', from, to);
  this.emit('all', 'set range', from, to);
  if (this.liveList) {
    this.list();
  }
};
